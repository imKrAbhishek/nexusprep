...
const generateQuiz = asyncHandler(async (req, res) => {
  ...
  if (!context?.trim()) return res.status(400).json({ success: false, message: 'context is required' });

  const quiz = await quizService.generateAndSaveQuiz({ title, context, category, courseId, difficulty });
  res.status(201).json({ success: true, message: 'Quiz generated successfully', data: { quiz } });
});

const getQuizzes = asyncHandler(async (req, res) => {
  const { category, courseId } = req.query;
  const quizzes = courseId ? await quizService.getQuizzesByCourse(courseId) : await quizService.getQuizzesByCategory(category);
...
const submitAttempt = asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(answers) || answers.length === 0) return res.status(400).json({ success: false, message: 'answers array is required' });
...
module.exports = { askDoubt, getMyDoubts, getDoubtSession, generateQuiz, getQuizzes, getQuizById, submitAttempt, getMyAttempts };